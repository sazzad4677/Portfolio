...
const Certifications = () => {
    const { certifications } = defaultContent;
    ...
    return (
        <Element name="certifications" className="scroll-anchor">
            <section id="certifications" className="py-24 overflow-hidden relative">
                {/* Background glow identical to archive/contact */}
                    ...
                    >
                        {certifications.map((cert) => (
                            <motion.div
                                ...
                                className="group flex flex-col h-full rounded-2xl border border-border/40 bg-surface/20 p-8 shadow-sm backdrop-blur-md transition-all duration-500 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5 relative overflow-hidden"
                            >
                                {/* Decorative line */}
                                ...
                                <div className="flex-1 flex flex-col">
                                    {cert.link ? (
                                        <a href={cert.link} target="_blank" rel="noopener noreferrer" className="group/title">
                                            <h3 className="text-xl font-bold text-foreground mb-2 leading-tight group-hover:text-primary group-hover/title:underline transition-colors duration-300">
                                                {cert.title}
                                            </h3>
                                        </a>
                                    ) : (
                                    ...
                                    <div className="flex items-center gap-4 text-sm font-mono text-primary/70 mb-4">
                                        <span className="font-semibold">{cert.issuer}</span>
                                        <div className="flex items-center gap-1.5 opacity-80 border-l border-primary/20 pl-4">
                                            <Calendar size={14} />
                                            <span>{cert.date}</span>
                                        </div>
                                    </div>

                                    <p className="text-secondary-foreground/80 text-sm leading-relaxed mt-auto">
                                        {cert.description}
                                    </p>
                                </div>
                            </motion.div>
