            ...
            >
                <div className="site-container">
                    ...
                    <div className="mb-12 sm:mb-16 md:mb-20 flex items-center space-x-4">
                        ...
                        <div className="h-px flex-1 bg-gradient-to-r from-border/60 to-transparent" aria-hidden="true" />
                    </div>
                    ...
                    <div className="mx-auto max-w-[900px]">
                        <div className="flex flex-col lg:flex-row">
                            ...
                            <div 
                                className="relative flex lg:flex-col overflow-x-auto lg:overflow-visible no-scrollbar border-b lg:border-b-0 lg:border-l border-border/40 pb-1 lg:pb-0"
                                role="tablist"
                            ...
                            >
                                {jobs?.map((job, index) => (
                                    <button
                                        ...
                                        aria-controls={`panel-${job.id || index}`}
                                        id={`tab-${job.id || index}`}
                                        className={`
                            ...
                            {/* Job Details */}
                            <div className="mt-6 sm:mt-8 lg:mt-0 lg:pl-12 flex-1 relative">
                                {jobs?.map((job, index) => (
                                    <div
                                        key={job.id || index}
                                        role="tabpanel"
                                        id={`panel-${job.id || index}`}
                                        aria-labelledby={`tab-${job.id || index}`}
                                        hidden={tabIndex !== index}
                                        className={tabIndex === index ? "block" : "hidden"}
                                    >
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 mb-5 sm:mb-7 md:mb-8">
                                                        <div>
                                                            <div className="flex items-center gap-2 sm:gap-3 mb-1">
                                                                <div className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg bg-primary/10 text-primary border border-primary/20" aria-hidden="true">
                                                            ...
                                                            <a
                                                                ...
                                                                aria-label={`Visit ${job.name} website (opens in a new tab)`}
                                                            >
                                                                @ {job.name}
